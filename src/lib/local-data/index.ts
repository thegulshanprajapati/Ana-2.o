...
const tokenUsageFilePath = path.join(dataPath, 'token-usage.json');
...
if (useMongoBackend) {
  getMongoClient().catch((err) => {
    console.warn('[Database] MongoDB connection check failed on startup. Falling back to local files.', err.message || err);
...
const sanitizeStoredMessage = (message: AppMessage): AppMessage => {
  const role: AppMessage['role'] = message.role === 'assistant' ? 'assistant' : 'user';
  const content = toSafeContentString(message.content);
  const createdAt =
...
const sanitizeChatSession = (session: ChatSession): ChatSession => {
  const id =
  ...
  const messages = Array.isArray(session.messages)
    ? session.messages.map((message) => sanitizeStoredMessage(message))
    : [];
...
export const getChatHistoryForUser = async (userId: string): Promise<ChatSession[]> => {
  ...
  for (const file of files) {
    ...
    if (session.id) {
      chatSessions.push(sanitizeChatSession(session));
    }
  }
...
export const saveChatHistory = async (
  ...
  const safeSession = sanitizeChatSession(session);

  if (useMongoBackend) {
    const db = await getMongoDb();
    await db.collection<MongoChatDoc>(COLLECTIONS.chats).updateOne(
      { userId, id: safeSession.id },
      {
        $set: {
          ...safeSession,
          userId,
  ...
  const userChatDir = getUserChatDir(userId);
  const sessionFilePath = path.join(userChatDir, `${safeSession.id}.json`);
  await writeJsonFile(sessionFilePath, safeSession);
};
...
export const deleteNotification = async (id: string): Promise<void> => {
  if (useMongoBackend) {
    const db = await getMongoDb();
    await db.collection<Notification>(COLLECTIONS.notifications).deleteOne({ id });
    return;
  }
  const allNotifications = await readJsonFile<Notification[]>(notificationsFilePath, []);
...
export const clearUserNotifications = async (userId: string): Promise<void> => {
  if (useMongoBackend) {
    const db = await getMongoDb();
    await db.collection<Notification>(COLLECTIONS.notifications).deleteMany({ userId });
    return;
  }
  const allNotifications = await readJsonFile<Notification[]>(notificationsFilePath, []);
  const updatedNotifications = allNotifications.filter((n) => n.userId !== userId);
  await writeJsonFile(notificationsFilePath, updatedNotifications);
};
...
export const saveTokenUsage = async (
  ...
  if (useMongoBackend) {
    const db = await getMongoDb();
    await db.collection<TokenUsageEntry>(COLLECTIONS.tokenUsage).insertOne(newEntry);
    return newEntry;
  }

  const allEntries = await getTokenUsage();
  allEntries.push(newEntry);
  await writeJsonFile(tokenUsageFilePath, allEntries);
  return newEntry;
};

export const getTokenUsage = async (): Promise<TokenUsageEntry[]> => {
  if (useMongoBackend) {
    const db = await getMongoDb();
    const entries = await db
      .collection<TokenUsageEntry>(COLLECTIONS.tokenUsage)
      .find({}, { projection: { _id: 0 } })
      .sort({ createdAt: -1 })
      .toArray();
    return entries;
  ...
  return readJsonFile<TokenUsageEntry[]>(tokenUsageFilePath, []);
};
