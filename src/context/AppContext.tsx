...
import { 
    ...
    clearUserNotifications as clearUserNotificationsAction,
    getFeedback,
...
interface AppContextType {
  ...
  toggleChatPinned: (id: string, pinned: boolean) => Promise<void>;
  toggleChatArchived: (id: string, archived: boolean) => Promise<void>;
  history: ChatSession[];
...
const sendPlanNotification = async (user: UserData, t: (key: TranslationKey) => string) => {
    ...
    if (title && description) {
        ...
        const duplicateExists = existing.some(n => n.title === title || n.description === description);
        if (!duplicateExists) {
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const t = useCallback((key: TranslationKey): string => {
    ...
    const handleClearNotifications = useCallback(async () => {
        if (!user) return;
        try {
            await clearUserNotificationsAction(user.id);
            setNotifications([]);
        } catch (error) {
            ...
            if (currentUser) {
                ...
                if (userHistory.length > 0) {
                    ...
                    const chat2: ChatSession = {
                        ...
                        messages: [{role: 'assistant', content: 'Here is another example chat.', createdAt: new Date().toISOString()}],
  ...
  const deleteChat = async (id: string) => {
    ...
    if (id === activeChatId) {
        if (updatedHistory.length > 0) {
            const nextChat = updatedHistory.find((chat) => !chat.isArchived) ?? updatedHistory[0];
            loadChatWrapper(nextChat.id);
        } else {
  ...
  const toggleChatPinned = async (id: string, pinned: boolean) => {
      setHistory(prevHistory => {
          const newHistory = prevHistory.map((chat) =>
          ...
          if (user) {
              const chatToSave = newHistory.find((chat) => chat.id === id);
              if (chatToSave) {
                  saveChatHistory(user.id, chatToSave);
              }
          }
  ...
  const toggleChatArchived = async (id: string, archived: boolean) => {
      if (!user) return;

      const updatedHistory = history.map((chat) =>
          chat.id === id ? { ...chat, isArchived: archived } : chat
      );

      setHistory(updatedHistory);

      const chatToSave = updatedHistory.find((chat) => chat.id === id);
      if (chatToSave) {
          await saveChatHistory(user.id, chatToSave);
      }

      if (archived && id === activeChatId) {
          const nextChat = updatedHistory.find((chat) => !chat.isArchived && chat.id !== id);
          if (nextChat) {
              loadChatWrapper(nextChat.id);
          } else {
              await startNewChat(user, language);
  ...
  return (
    <AppContext.Provider value={{ 
        theme, setTheme, 
        messages, addMessage, setMessages, startNewChat: startNewChatWrapper, loadChat: loadChatWrapper, history, activeChatId, updateChatTitle, deleteChat, toggleChatPinned, toggleChatArchived,
