...
const PUBLIC_CHAT_ERROR_MESSAGE = 'Abhi usage high hai. Thodi der baad try karo.';
...
const isAdminByUserId = async (userId: string): Promise<boolean> => {
  if (!userId) {
    return false;
  }
  ...
  try {
    ...
    console.error('[api/chat] admin_lookup_failed', error);
    return false;
  }
};
...
export const handleChatPost = async (
  ...
  try {
    ...
    if (error instanceof HuggingFaceError) {
      const isAdminRequest = await isAdminByUserId(requestingUserId);
      return NextResponse.json(
        {
          error: isAdminRequest ? error.message : PUBLIC_CHAT_ERROR_MESSAGE,
        },
        { status: 400 }
      );
    }
